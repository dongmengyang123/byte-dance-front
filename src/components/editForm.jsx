...
import { userAdd, userSearch, userEdit } from '../api/user.js';
...
function ModalForm(props) {
  ...
  const openModal = async () => {
    setIsModalOpen(true);
    // 查询回显
    const result = await userSearch(id);
    if (result.data.code) {
  ...
  const handleSubmit = async (e) => {
    ...
    // 校验通过，发起接口请求
    try {
      const result = await userEdit(formData);
      // 假设接口返回 code 为 truthy 时表示成功（根据实际接口调整）
      if (result.data.code) {
        message.success('修改成功');
        search();
      } else {
        message.error('修改失败：' + (result.data.msg || '未知错误'));
      }
    } catch (error) {
  ...
  return (
    <div>
      <button onClick={openModal} className={cls('edit-btn')}>
        编辑
      </button>
            ...
            <form onSubmit={handleSubmit} className={cls('modal-form')}>
              ...
              <div className={cls('form-item')}>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  onBlur={() => handleBlur('createTime')} // 失去焦点校验
                  required
                />
                {/* 错误提示 */}
                {errors.createTime && (
                  <span className={cls('error-message')}>{errors.createTime}</span>
                )}
              </div>
